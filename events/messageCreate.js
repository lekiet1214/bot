...
module.exports = {
  ...
  run: async (message, client) => {
    ...
    const replyString = `${message.author.username} +  in channel ' + ${message.channel.name} + ' at ' ${Date.now().toString()} + ' sent: ' + ${message.content}\n`
    console.log(replyString)
    client.logger.log(replyString)
    if (command === 'ping') {
    ...
    if (command === 'purge') {
      ...
      message.channel.send(`${message.author.username} has purged ${deleteCount} messages.`)
    }
  }
}
